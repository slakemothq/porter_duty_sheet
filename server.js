...
// ── Middleware ─────────────────────────────────────────────────────
// Increased limit to 50mb to handle multiple base64 encoded photos
...
// ── Get all user data keys (used to load all photos) ──────────────
app.get('/api/data', requireAuth, async (req, res) => {
  try {
    const prefix = req.session.userId + '_';
    const rows = await dbFind(data, { userKey: new RegExp('^' + prefix.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')) });
    const result = {};
    rows.forEach(r => {
      const key = r.userKey.replace(prefix, '');
      result[key] = r.value;
    });
